...
module.exports = {
  ...
  addExpense: function(pool, expense, cb){
    let expenseSql  = `insert into expenses set ?`
    let newExpense  = {
      day         : expense['day'],
      month       : expense['month'],
      year        : expense['year'],
    ...
    pool.getConnection(function(getConnectionErr, connection){
      ...
      connection.query(expenseSql, newExpense, function(expenseQueryErr, addExpenseResults, fields){
        connection.release()
        if (expenseQueryErr){
          ...
          cb('', {expense_id: addExpenseResults.insertId})
        }
      })
    })
  },
  ...
  updateExpense: function(pool, expense, cb){
    let updateExpSql  = `update expenses set ? where expense_id = ${expense['expenseId']} and sheet_id = ${expense['sheetId']}`
    let updatedExpense  = {
      day         : expense['day'],
      month       : expense['month'],
      year        : expense['year'],
    ...
    pool.getConnection(function(getConnectionErr, connection){
      ...
      connection.query(updateExpSql, updatedExpense, function(updateQueryErr, updateExpenseResults, fields){
        connection.release()
        if (updateQueryErr){
          ...
          cb('', {expense_id: updateExpenseResults.insertId})
        }
      })
    })
  },
  ...
  deleteExpense: function(pool, expenseId, cb){
    let deleteExpSql  = `delete from expenses where expense_id = ${expenseId}`
    ...
    pool.getConnection(function(getConnectionErr, connection){
      ...
      connection.query(deleteExpSql, function(deleteQueryErr, delExpenseResults, fields){
        connection.release()
        if (deleteQueryErr){
          ...
          cb('', {expense_id: delExpenseResults.insertId})
        }
      })
    })
  },
  ...
  getExpenses: function(pool, year, mon, sheetId, cb){
    let getExSql  = `select expense_id, day, month, year, category, subcategory, amount, paymentType as mode, card as cardtype, notes, sheet_id from expenses where year = ${year} and month = ${mon} and sheet_id = ${sheetId}`
  ...
  getYears: function(pool, sheetId, cb){
    let getYearsSql  = `select distinct(year) as year from expenses where sheet_id = ${sheetId} order by year desc`
  ...
  getCategory: function(pool, sheetId, cb){
    let getCatSql  = `select distinct(category) as category from expenses where sheet_id = ${sheetId} order by category`
  ...
  getSubCategory: function(pool, sheetId, cb){
    let getSubCatSql  = `select distinct(subcategory) as subcategory from expenses where sheet_id = ${sheetId} order by subcategory`
